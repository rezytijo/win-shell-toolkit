...
<#
.SYNOPSIS
Interactive TUI generating an offline Wi-Fi QR code graphic
...
function Invoke-WiFiQR {
    ...
    $selected = Show-Menu -Options $profiles -Title "WI-FI INSTANT SHARING (QR CODE)"
    ...
    if ($password -eq "") {
         Write-Host "  [Warning] Network $selected seems to be an OPEN network." -ForegroundColor Yellow
         $authType = "nopass"
    } else {
         $authType = "WPA" # Works well enough for most QR code readers 
    }

    Write-Host "  [OK] Network   : $selected" -ForegroundColor Green
    Write-Host "  [OK] Password  : (Hidden / Extracted)" -ForegroundColor Green
    Write-Host ""
    Write-Host "  Generating offline Wi-Fi QR Code..." -ForegroundColor DarkGray
    Write-Host ""
    ...
    try {
        ...
        $outPath = Join-Path $env:TEMP "wifi_qr_$(Get-Date -Format 'yyyyMMdd_HHmmss').png"
        
        # New-PSOneQRCodeWifiAccess builds the WIFI:S: string properly on its own
        if ($authType -eq "nopass") {
            New-PSOneQRCodeWifiAccess -SSID $selected -Width 400 -OutPath $outPath -Show -ErrorAction Stop
        } else {
            New-PSOneQRCodeWifiAccess -SSID $selected -Password $password -Width 400 -OutPath $outPath -Show -ErrorAction Stop
        }
        
        Write-Host "  [Success] High-Res QR Code graphic opened in your image viewer!" -ForegroundColor Cyan
    } catch {
